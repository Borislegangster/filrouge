...
export default function Acquisitions({ darkMode }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const fetchAcquisitions = async () => {
      try {
        ...
        const pending = response.data.data.filter((a) => a.status === 'En attente').length;
        const approved = response.data.data.filter((a) => a.status === 'Validée').length;
        const rejected = response.data.data.filter((a) => a.status === 'Rejetée').length;
        const delivered = response.data.data.filter((a) => a.status === 'Livrée').length;
        const totalAmount = response.data.data
          .filter((a) => a.status === 'Validée')
  ...
  const handleApprove = async (acquisitionId, amount) => {
    try {
  ...
  const handleReject = async (acquisitionId, reason) => {
    try {
      await api.put(`/api/acquisitions/${acquisitionId}`, {
        status: 'Rejetée',
        description: reason
