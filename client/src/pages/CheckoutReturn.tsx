...
export default function CheckoutReturn({ darkMode }: CheckoutReturnProps) {
  ...
  const [sortConfig, setSortConfig] = useState<{ key: keyof Checkout; direction: 'ascending' | 'descending' } | null>(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState<string>('');

  const requestSort = (key: keyof Checkout) => {
  ...
  const getSortIcon = (key: keyof Checkout) => {
  ...
  const getStatusBadgeClass = (status: Checkout['status']) => {
    switch (status) {
      case 'En cours':
