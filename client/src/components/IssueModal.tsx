...
export default function IssueModal({ isOpen, onClose, onSave, issue, darkMode }: IssueModalProps) {
  const { user } = useAuth();
            ...
            <form onSubmit={handleSubmit} className="mt-4 space-y-4">
              <div>
                ...
                <input
                  ...
                  className={`mt-1 block w-full rounded-md shadow-sm ${darkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300'} focus:ring-blue-500 focus:border-blue-500 sm:text-sm`}
                />
              </div>
              <div>
              ...
              <div>
                <label htmlFor="equipment" className={`block text-sm font-medium ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                  Équipement
                </label>
                <select
                  id="equipment"
